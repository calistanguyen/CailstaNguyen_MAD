...
class SearchTable: UITableViewController, UISearchResultsUpdating {
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        filteredPokemon.removeAll(keepingCapacity: true) //removes all elements
        if searchString?.isEmpty == false {
            ...
            let matches = allPokemon.filter(searchfilter)
            filteredPokemon.append(contentsOf: matches)
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cellConfig.text = filteredPokemon[indexPath.row]
        cell.contentConfiguration = cellConfig
        return cell
    }
 
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            let alert = UIAlertController(title: "Row selected", message: "You selected \(filteredPokemon[indexPath.row])", preferredStyle: .alert)
